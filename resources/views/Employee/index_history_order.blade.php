        ...
        @for($i = 1; $i <= $total_days; $i++, $start->addDay())
          ...
          <li>
            <a href="#">
              ...
              <div class=" text-xl font-bold text-gray-600 text-left px-2">{{$order->menu_name}}</div>
              <div class=" text-base font-base text-gray-600 text-left px-2">@if($order->menu != null) {{$order->menu->desc}} @endif</div>     
              <div class=" text-base font-base text-orange-500 text-right px-2">@if($order->menu != null) @for($k = 1; $k <= $order->menu->orders->avg('stars'); $k++) <i class="fas fa-star"></i>@endfor @endif</div>      
              </div>
            </a>
          </li>
