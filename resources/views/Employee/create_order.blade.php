   ...
   <div class="flex-row gap-2 row-span-3 px-4 mb-8 row-span-5 mt-4">                      
     <div id="div1" class="duration-1000 targetDiv justify-content content-center md:text-left text-center pt-4 md:mx-4 lg:mx-auto" style="width: fit-content;width:-moz-fit-content;"> 
      ...
      <label class='label flex-auto contents duration-1000'>
        <input class='label__checkbox duration-1000 ' name="dates[]" type='checkbox' id="tanggal{{$start->format('Y-m-d')}}"  @if($start->day == $now->day && $total_dadakan <= 5 && $schedule != null && $now->format('H') <= 9) @elseif((Carbon\Carbon::parse($user->created_at)->format('Y-m') == $start->format('Y-m')) && ($now->lte($start)) && $schedule != null && $user->can_order == 1) @elseif($order != null && $start > $now) @elseif(!$user->can_order_directly && ($schedule != null && $start > $now)) @elseif($schedule != null) disabled @elseif($schedule == null || $start < $now) disabled @endif value="{{$start->format('Y-m-d')}}" onclick="document.getElementById('tanggal{!! $start->format("Y-m-d") !!}').checked = false;" data-toggle="modal" data-target="#ScheduleModal">
